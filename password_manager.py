...
def master():
    # Create an account on twilio website and activate the sandbox with your number
    # Then Enter your account SSID and Auth Token which will be available in your consolw
    client = Client(account ssid,auth token)
    from_whatsapp_number='whatsapp:+14155238886' # This is one of the numbers given by twilio
    to_whatsapp_number=''
    alpha = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890!@#$%^&*()_=+,./;}[]{:<>?'
    lst = list(alpha)
    pas = []
    for x in range(10):
    	pas.append(lst[randint(0,len(lst)-1)])
    passg = ''.join(pas)
    client.messages.create(body=passg,
